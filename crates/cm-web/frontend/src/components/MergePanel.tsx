...
export function MergePanel({ entries, onComplete, onCancel }: MergePanelProps) {
  ...
  const handleMerge = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        `Merged ${entries.length} entries, kept "${baseEntry.title}"`,
      );
      onComplete();
    } catch {
    ...
    <div className="space-y-4 rounded-lg border border-border bg-card p-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="font-mono text-[10px] text-muted-foreground">
            Select the base entry to keep. Others will be superseded. Tags are
            combined.
          </p>
        </div>
        <button
      ...
      <div className="flex items-center gap-3 border-t border-border pt-3">
        ...
        <span className="font-mono text-[10px] text-muted-foreground">
          {othersToSupersede.length} {othersToSupersede.length === 1 ? "entry" : "entries"} will be superseded
        </span>
      </div>
    </div>
  );
